...
def liter(key: str):
    ...
    if lang == "en":
        s = {ru: en for ru, en in zip(rukeys, enkeys)}
        try:
            return s[key]
        except:
            return key
    else:
        s = {en: ru for ru, en in zip(rukeys, enkeys)}
        try:
            return s[key]
        except:
            return key
...
class Input:
    ...
    def check(self) -> list[str]:
        keys = []
        for key in list("`1234567890-=qwertyuiop[]asdfghjkl;'\\zxcvbnm,./") + ["f1", "f2", "f3", "f4", "f5", "f6", "f7", "f8", "f9", "f10", "f11", "f12", "insert", "home", "delete", "end", "tab", "caps lock", "shift", "esc", "ctrl", "alt", "backspace", "enter", "space"]:
