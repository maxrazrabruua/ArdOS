...
class Window:
    def __init__(self, style: Style, xy: tuple[int, int], wh: tuple[int, int], title: str, script: list[dict] = [], pyscripts: tuple[str, str] = ("", "", ""), id: int = 0):
        self.style = style
        self.xy = xy
        self.wh = wh
        self.title = title
        ...
        if not update:
            ...
            for command in self.script:
                if command["command"] == "text":
                    ...
                    Text((xy[0] + self.xy[0] + 10, xy[1] + self.xy[1] + y), width, text, color)()
                    y += 3 + width
...
class WindowMeneger:
    ...
    def __call__(self, phone: tuple[int, int, int] | None = None):
        if phone:
            screen.fill(phone)
