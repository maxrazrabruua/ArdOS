...
class WindowMeneger:
    ...
    def append(self, window: Window):
        window.id = self.windows[len(self.windows.keys()) - 1].id + 1 if len(self.windows.keys()) else 0
        window.add['meneger'] = self
        self.windows[self.windows[len(self.windows.keys()) - 1].id + 1 if len(self.windows.keys()) else 0] = window
