...
class Input:
    class Filter:
        ...
        def ready(self, keys: list[str]):
            l = []
            for key in keys:
                if key in list("1234567890qwertyuiopasdfghjklzxcvbnm"):
                    l.append(key)
                else:
                    tk = [41, 26, 27, 39, 40, 51, 52, 53, 43, 12, 13, "space"]
                    tv = list("`[];',./\\-= ")
                    t = {k: v for k, v in zip(tk, tv)}
                    try:
                        l.append(t[key])
                    except:
                        pass
    ...
    def check(self) -> list[str]:
        keys = []
        for key in list("1234567890qwertyuiopasdfghjklzxcvbnm") + [41, 26, 27, 39, 40, 51, 52, 53, 43, 12, 13, "space", "f1", "f2", "f3", "f4", "f5", "f6", "f7", "f8", "f9", "f10", "f11", "f12", "insert", "home", "delete", "end", "tab", "caps lock", "shift", "esc", "ctrl", "alt", "backspace", "enter", "space"]:
